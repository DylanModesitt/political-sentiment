...
class TwitterClient(object):
    ...
    def get_senate_dems(self):
        return [user.screen_name for user in tweepy.Cursor(self.api.list_members, "TheDemocrats", "senate-democrats").items()]

    def get_senate_gop():
        return [user.screen_name for user in tweepy.Cursor(self.api.list_members, "SenateGOP", "senaterepublicans").items()]

    def get_house_dems():
        return [user.screen_name for user in tweepy.Cursor(self.api.list_members, "TheDemocrats", "house-democrats").items()]

    def get_house_gop():
        return [user.screen_name for user in tweepy.Cursor(self.api.list_members, "housegop", "house-republicans").items()]
